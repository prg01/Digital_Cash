...
def Process_Money_Order(MO,k,rand2):
    ...
    for i in range (0, k):
        if i not in rand2:
            signed = i
            MO_SIGN = Sign_Money_order(signed, amount[i])
    return VERIFY, MO_SIGN
...
def Sign_Money_order(signed, amount):
    message_s_str = str(signed) + str (amount)
    print (message_s_str)
...
def Verify_Money_Order(Msg):
    ciphertext    = base64.decodebytes(Msg)
    ...
    message_ID = cipher.decrypt(ciphertext_ID)
    id1 = str(message_I1,'utf-8') 
    id2 = str(message_I2,'utf-8') 
    id3 = str(message_I3,'utf-8') 
    message_4 = str(message_ID,'utf-8') 
    unique_id = message_4[:100]
    amount    = message_4[100:]
